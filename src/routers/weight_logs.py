...
async def find_log(log_id: UUID, session: AsyncSession) -> WeightLog:
...
async def create_weight_log(
    payload: WeightLogCreate, session: DatabaseSession
) -> WeightLog:
...
async def list_weight_logs(session: DatabaseSession) -> list[WeightLog]:
    return list(
        await session.scalars(select(WeightLog).order_by(WeightLog.date.desc()))
    )


@router.get("/{log_id}", response_model=WeightLogRead)
async def get_weight_log(log_id: UUID, session: DatabaseSession) -> WeightLog:
    return await find_log(log_id, session)
...
async def update_weight_log(
    ...
    log = await find_log(log_id, session)
...
async def delete_weight_log(log_id: UUID, session: DatabaseSession) -> Response:
    await session.delete(await find_log(log_id, session))
    await session.commit()
